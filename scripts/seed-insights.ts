 ...
 *
 * Idempotent: re-running updates the same documents in place, and pushes the
 * current cover PNG over the one already in the media library, so a repaint of
 * the cover art (pnpm generate:covers) reaches the site on the next seed.
 *
...
for (const post of posts) {
  ...
  // The cover art is regenerated by pnpm generate:covers, so the file on disk
  // is the source of truth and every run pushes it back into the library:
  // repainting the covers to a new palette otherwise leaves the old art live.
  // Drop the previous document first rather than updating it in place, because
  // an update writes a second file beside the old one and takes a deduplicated
  // filename ("...-1.png"), which orphans the original and mangles the URL.
  const coverPath = path.join(root, `scripts/content/covers/${post.slug}.png`);
  const previousCoverId = found?.coverImage
    ? typeof found.coverImage === "object"
      ? (found.coverImage as { id: number | string }).id
      : (found.coverImage as number | string)
    : undefined;
  if (previousCoverId) {
    await payload.delete({ collection: "media", id: previousCoverId });
  }
  const coverId = (
  ...
  console.log(`  ${previousCoverId ? "↻ replaced" : "↑ uploaded"} cover for ${post.slug}`);
